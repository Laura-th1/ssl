...
if($_POST['opcion'] == 'AccionConsultar'){
    ...
    $consulta = "SELECT * FROM roles WHERE descripcion = '" . $nombre . "'";
    $data_con = $con->query($consulta);
    ...
    if($data_con->num_rows == 0){
        ...
        $consulta = "INSERT INTO roles (descripcion, estado, usuario_create, usuario_act, fecha_create, fecha_act)
                            VALUES ('". $nombre ."', 1, 1, NULL, CURRENT_TIMESTAMP, CURRENT_DATE)";
        $data =  $con->query($consulta);

        if (!$data) {
            $alerta = "ERROR";
            $mensaje = "Error al insertar el rol: " . $con->error;
        }
    }else{
        $alerta                     =                   "ERROR";
